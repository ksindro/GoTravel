...
function LlenarCarrito(){
    ...
    for(let index = 0; index < carrito.length; index++){
        ...
        scriptTabla +=  '<img src= "../images/eliminar2.png" alt = "eliminar" onclick = "eliminar('+index+')">';
        scriptTabla += '</div>';
        scriptTabla += '</td>';
...
function ExistenciaSuficiente(idPedido){
    for(let i = 0; i<productos.length ;i++){
        if(idPedido == productos[i].codigo){
            if(document.getElementById("idCantidad"+idPedido).value > productos[i].existencia){
                return false;
            }
            else{
                return true;
...
function restar(i){
    if(carrito[i].cantidad >1){
      carrito[i].cantidad--;  
        ...
        alert("Debe contar con 1 pedido");
    }
...
function confirmarPedido(){
    ...
    for(let i = 0 ; i<carrito.length ; i++){
        for(let u = 0; u<productos.length; u++){
            if(carrito[i].producto.codigo == productos[u].codigo){
                if(carrito[i].cantidad < productos[u].existencia){
                    alert("Del producto "+carrito[i].producto.nombre+" tenemos "+productos[u].existencia+" existencias");
                    return;
                }
            }
        }
    ...
    for(let i = 0 ; i<carrito.length ; i++){
        for(let u = 0; u<productos.length; u++){
            if(carrito[i].producto.codigo == productos[u].codigo){
                productos[u].existencia = parseInt(productos[u].existencia) - parseInt(carrito[i].cantidad);
            }
        }
    }
    alert("El pedido se ha confirmado exitosamente")
    setProducto();
    localStorage.removeItem("LSPedido");
    carrito = [];
